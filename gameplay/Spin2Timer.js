...
const {PreviousWinData,getUserPoints,calcWinningPoints,JoinGame,setRoundCount,calcWinningNo} = require("../services/Spin2GameService");
...
function OnSendTimer(data){
	...
	socket.on(events.OnTimer, async (data) => {
		...
		let points = await getUserPoints(data.user_id)
		let previousWinData = await PreviousWinData(RoundCount); 
		let obj = {
			balance : points,
			gametimer:timer, 
    	...
    	socket.emit(events.OnTimer,obj);
    })
}
